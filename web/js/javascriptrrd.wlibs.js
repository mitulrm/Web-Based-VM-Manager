...
rrdFlotMatrix.prototype.createHTML = function() {
  ...
  if(this.rrdflot_defaults.graph_height!=null) {
     elGraph.style.height=this.rrdflot_defaults.graph_height;
  } else {elGraph.style.height="600px";}
  elGraph.id=this.graph_id;
  cellGraph.appendChild(elGraph);
...
function timeoutrrdFlotAsyncCallback(bf,obj) {
  var i_rrd_data=undefined;
  if (bf.getLength()<1) {
    alert("File "+obj.url+" is empty (possibly loading failed)!");
    return 1;
...
rrdFlotAsync.prototype.timeoutreload = function(url) {
  this.url=url;
  try {
    FetchBinaryURLAsync(url,timeoutrrdFlotAsyncCallback,this);
  } catch (err) {
    alert("Failed loading "+url+"\n"+err);
  }
...
rrdFlotAsync.prototype.timeoutcallback = function() {
  ...
  this.rrd_flot_obj.rrd_file = irrd_data;
  
  
  graph_info={};
  this.rrd_flot_obj.createHTML();
  this.rrd_flot_obj.populateRes();
  this.rrd_flot_obj.populateDScb();
  this.rrd_flot_obj.drawFlotGraph();
  
  
  if (this.rrd_flot_obj.rrdflot_defaults.graph_only==true) {
    this.rrd_flot_obj.cleanHTMLCruft();
  }
};
/*
function rrdFlot(html_id, rrd_file, graph_options, ds_graph_options, rrdflot_defaults) {
